...
public class SelectIterator<T, ID> implements CloseableIterator<T> {
	...
	private T getCurrent() throws SQLException {
		if(rowMapper instanceof BaseMappedStatement){
			Object object = ((BaseMappedStatement) rowMapper).getTableInfo().createObject();
			if(object instanceof BaseDbBean){
				last= (T) ((BaseDbBean) object).fillDataBuCursor(results.getCursor());
			}
		}
		if(last==null){
